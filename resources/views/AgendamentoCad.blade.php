...
<nav class="relative px-4 py-4 flex justify-between items-center bg-white">
		...
		<li><a class="text-sm text-black-600 hover:text-gray-500" href="welcome.blade.php">Home</a></li>
            ...
            <li><a class="text-sm text-black-400 hover:text-gray-500" href="#Sobre">Sobre Nós</a></li>
            <li class="text-gray-300">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" class="w-4 h-4 current-fill" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 5v0m0 7v0m0 7v0m0-13a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2z"></path>
                </svg>
    ...
    <div class="w-full  p-6">
            <h1 class="text-3xl font-semibold mb-6 text-black text-center">Agende seu Horário!!!</h1>
            <p class="my-4 text-center">Faça seu Agendamento de Atendimento para o Beleza em fios!</p>
            <form method="POST" action="/adicionarA">
            @csrf
  <!-- Serviços -->
  <div class="form-group  mb-2">
                ...
                <select class="custom-select" name="Servico">
                    ...
                    <option value="3">Tratamento de Pele</option>
                    <option value="4">Massagem</option>
                </select>
            </div>
            <!-- Data do Serviço -->
            <div class="form-group  mb-2">
            ...
            <div class="form-group  mb-2">
                ...
                <input type="text" class="form-control" name="IdFunc">
            </div>
            <br>
            <br>
            ...
            @if(isset($agendamentos) && count($agendamentos) > 0)
                    @foreach ($agendamentos as $agen)
                        <tr>
                            <th>{{ $agen->id }}</th>
                            <th>{{ $agen->Servico }}</th>
                            <th>{{ $agen->DataServico }}</th>
                            <th>{{ $agen->Horario }}</th>
                            <th>{{ $agen->IdCliente }}</th>
                            <th>{{ $agen->IdFunc }}</th>

                            <th><a href="/editarA/{{ $agen->id }}" class="btn btn-primary">Editar</a>
                                <a href="/excluirA/{{ $agen->id }}" class="btn btn-danger">Excluir</a>
                            </th>
                        </tr>
                    @endforeach
                @else
